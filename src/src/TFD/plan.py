...
def main():
	...
	# run translator
	run("downward/translate/translate.py", domain, problem)

	# run preprocessing
	run("downward/preprocess/preprocess", input="output.sas")

	# run search
	run("downward/search/search", config, "p", result_name, input="output")
